...
public class Parser {
    ...
    //  op ::=  `+` | `-` | `*` | `/` | `&&` | `||` | `<` | `>`     // arithmetic, logical, and comparison operators
    ...
    // exp ::= `(` exp op exp `)`     // Arithmetic expressions
    public ParseResult<Exp> parseArithmeticExpressions(final int p) throws ParseException{
        ...
        ParseResult<Op> op = parseOp(exp1.nextP);
    ...
    } // parse arithmetic exp


    // exp ::= `(` vars `)` `EXECUTES` exp     // defines higher-order functions
    ...
    // exp ::= `CALL` exp `(` exps `)`     // calls higher-order function
    ...
    // exp ::= `PRINT` exp     // prints to terminal, returns a number
    ...
    // stmt ::= `WHILE` `(` exp `)` stmt     // loop statements
    ...
    // stmt ::= `IF` `(` exp `)` stmt `ELSE` `stmt`     // if-else statements
    ...
    // stmt ::= exp`;`     // expressions can be statements
    ...
    // stmt ::= `DEFINE` type methodName `(` params `)` `{` stmt* `}`       // method declaration
    public ParseResult<Stmt> parseMethodDef(final int p) throws ParseException {
